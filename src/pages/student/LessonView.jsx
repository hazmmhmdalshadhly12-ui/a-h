...
export default function LessonView() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([l, h, f, c]) => {
        if (!active) return;
        const errs = [l?.error, h?.error, f?.error, c?.error].filter(Boolean);
        if (errs.length) {
          setLoadError(errs.map((e) => e.message || String(e)).join(' | '));
        }
        ...
        if (!lessonId && lessonList.length > 0) {
          const firstAccessible = lessonList.find((x) => x.accessible || x.is_free);
          if (firstAccessible) {
            navigate(`/student/courses/${courseId}/lesson/${firstAccessible.lesson_id}`, { replace: true });
          }
        }
  ...
  if (!course) {
    return (
      <Card className="flex flex-col items-center gap-4 py-14 text-center">
        ...
        <p className="text-xs text-muted">courseId: {courseId}</p>
  ...
  return (
    <div className="space-y-6">
      ...
      {isProfessional && !canWatch && !effectiveAccess ? (
          ...
          {showSubscribe && (
            <div className="space-y-4 rounded-lens border border-signal/40 bg-signal/10 p-4">
              ...
              <form onSubmit={submitSubscription} className="grid gap-3 sm:grid-cols-2">
                <input name="parent_phone" dir="ltr" placeholder="01xxxxxxxxx" value={subForm.parent_phone} onChange={(e) => setSubForm({ ...subForm, parent_phone: e.target.value })} className="input-base" />
                <input name="transfer_number" dir="ltr" placeholder="01xxxxxxxxx" value={subForm.transfer_number} onChange={(e) => setSubForm({ ...subForm, transfer_number: e.target.value })} className="input-base" required />
        ...
        <>
          <div className="grid gap-6 lg:grid-cols-[260px_1fr]">
            <Card className="max-h-[calc(100vh-6rem)] overflow-y-auto lg:sticky lg:top-6">
                          ...
                          >
                            ...
                            <span className="min-w-0 flex-1">
                              ...
                              <span className="block text-xs text-muted">
                                ...
                                {l.video_url ? (accessible ? 'فيديو' : 'فيديو مقفول') : ''}
              ...
              <section className="space-y-4">
                ...
                <Card className="space-y-4">
                  <form onSubmit={submitUpload} className="grid gap-3 sm:grid-cols-3">
                    <input name="upload_title" placeholder="مثال: حل التمارين" value={uploadTitle} onChange={(e) => setUploadTitle(e.target.value)} className="input-base" />
              ...
              <section className="space-y-4">
                ...
                <Card className="space-y-4">
                  <form onSubmit={submitComment} className="flex flex-col gap-2 sm:flex-row sm:items-end">
                    <textarea name="comment" rows={2} placeholder="اسأل أو شارك معلومة..." value={commentBody} onChange={(e) => setCommentBody(e.target.value)} className="input-base min-h-[80px] resize-y" />
      ...
      {loadError && (
        <Card className="border border-danger/40 bg-danger/5 p-4">
          <p className="text-sm font-bold text-danger">تعذر تحميل جزء من بيانات الدرس</p>
          <p dir="ltr" className="mt-1 font-mono text-xs text-muted">{loadError}</p>
        </Card>
      )}
    </div>
  );
